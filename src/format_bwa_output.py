...
if genome_dir == None or fastq_dir == None or output_dir == None:
    ...
    for file in os.listdir(str(fastq_dir)): #loop through files in data directory
        if file.endswith('.sam'): #if file is sam file
            ...
            cmd = 'samtools view -b -F4 -o ' + str(output_dir) + str(base) +  '.bam -@ 4 ' + r
            os.system(cmd)
            os.system('samtools sort ' + str(output_dir) + str(base) + '.bam > ' + str(output_dir) + 'sorted_' + str(base) + '.bam')
            os.system('samtools index ' + str(output_dir) + 'sorted_' + str(base)+ '.bam')
