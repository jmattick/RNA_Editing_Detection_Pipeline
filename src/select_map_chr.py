...
fasta_dir = None # path to directory containing WGS fasta files and also sam file
...
for i in range(len(params)-1): #loop through parameters
    ...
    elif params[i] == '-f' or params[i] == '--fasta_dir': #if parameter is the WGS fasta file directory
...
if genome_dir == None or fasta_dir == None or chrNum == None or output_dir == None:
	...
	for file in os.listdir(str(fasta_dir)): #loop through files in data directory
        if file.endswith('.sam'): #if file is sam file
            ...
            r = str(fasta_dir) + str(base) + '.sam' #path to sam file
